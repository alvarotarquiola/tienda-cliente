...
function imageResize($sourceFile, $destFile, $destWidth = NULL, $destHeight = NULL, $fileType = 'jpg')
{
	...
	{
		global $pnconfig;
		if(isset($pnconfig["metodo"])){
			$confi = $pnconfig["metodo"];
		}else{
			$confi = Configuration::get('PS_IMAGE_GENERATION_METHOD');
		}
		
		if (intval($confi) == 2 OR (intval($confi) == 0 AND $widthDiff > $heightDiff))
		{
			$nextHeight = $destHeight;
			$nextWidth = intval(($sourceWidth * $nextHeight) / $sourceHeight);
			$destWidth = (intval($confi) == 0 ? $destWidth : $nextWidth);
		}
		else
		{
			$nextWidth = $destWidth;
			$nextHeight = intval($sourceHeight * $destWidth / $sourceWidth);
			$destHeight = (intval($confi) == 0 ? $destHeight : $nextHeight);
		}
	}
