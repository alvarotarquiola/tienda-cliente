...
class Mail
{
	static public function Send($id_lang, $template, $subject, $templateVars, $to, $toName = NULL, $from = NULL, $fromName = NULL, $fileAttachment = NULL, $modeSMTP = NULL, $templatePath = _PS_MAIL_DIR_)
	{
		...
		if (
			(!empty($from) AND !Validate::isEmail($from)) 
			OR 
			(!empty($fromName) AND !Validate::isMailName($fromName)) 
			//OR 
		 	//(!is_array($to) AND !Validate::isEmail($to)) 
		 	OR 
		 	(!empty($toName) AND !Validate::isMailName($toName)) 
		 	OR 
		 	!is_array($templateVars) 
		 	OR 
		 	!Validate::isTplName($template) 
		 	OR 
		 	!Validate::isMailSubject($subject)
		 	)
	 		die(Tools::displayError('Error: mail parameters are corrupted 1'));
		...
		if (is_array($to))
		{
			...
			foreach ($to as $key => $addr)
			{
				...
				if (!Validate::isEmail($addr))
					die(Tools::displayError('Error: mail parameters are corrupted 2'));
