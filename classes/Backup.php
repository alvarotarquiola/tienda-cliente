...
class Backup
{
	...
	public function bakcup_true()
	{
		...
		foreach ($tables as $table)
		{
            ...
            //Drop table
            $dropTable = 'DROP TABLE '.$schema[0]['Table'].';buffernowdotcom';
            
            fwrite($fp, $dropTable . "\n\n");

			fwrite($fp, '/* Scheme for table ' . $schema[0]['Table'] . " */\n");
			//fwrite($fp, $schema[0]['Create Table'] . ";\n\n");
            fwrite($fp, $schema[0]['Create Table'] . ";buffernowdotcom\n\n");
			...
			if (!in_array($schema[0]['Table'], $ignore_insert_table))
			{
                ...
                $row23 = DB::getInstance()->nextRow($data);
    ...
    public function add()
    {
        ...
        /*desde aqui otra forma de bakcup*/
        ...
        $data = fread ($fp, filesize($backupfile));
        fclose($fp);
        $zp = gzopen($backupfile.".gz", "w9");
        gzwrite($zp, $data);
        gzclose($zp);
    
        
        $dirdelFile = PS_ADMIN_DIR."/backups/".$aux_filename;
