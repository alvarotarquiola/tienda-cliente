...
class		Cart extends ObjectModel
{
	...
	*/
	...
	public function OrderExists()
	{
		...
		$salida = Db::getInstance()->NumRows();
		
		
		//echo "<br>salida ->".$salida;
		//echo "result:";
		//print_r($result);
		//echo "<br>id ->".intval($this->id);
		...
		if($paso && ($salida >= 1)){
			...
			/*
			$ordertm = new Order(intval($this->id));
			//print_r($ordertm);
			//if (Validate::isLoadedObject($ordertm) AND $ordertm->id_customer == $cookie->id_customer)
				//{
					//echo "entraaa";
					//$id_order_state = intval($ordertm->getCurrentState());
					$id_order_state = intval(OrderHistory::getLastOrderState(intval($this->id)));
					echo "<br>order-state -> ".$id_order_state;
					//if($id_order_state==_PS_OS_PAYPAL_){
						$salida = 0;
					//}
				//}
			*/	
	...
	*/
	...
	public function checkQuantitiesv2()
	{
		foreach ($this->getProducts() AS $product)
