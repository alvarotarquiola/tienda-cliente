...
class		Language extends ObjectModel
{
	...
	protected 	$fieldsRequired = array('name', 'iso_code', 'iso_lang');
	protected 	$fieldsSize = array('name' => 32, 'iso_code' => 3, 'iso_lang' => 32);
	protected 	$fieldsValidate = array('name' => 'isGenericName', 'iso_code' => 'isLanguageIsoCode', 'iso_lang' => 'isGenericName', 'active' => 'isBool');
	...
	public function getFields()
	{
        ...
        $fields['iso_lang'] = pSQL($this->iso_lang);
    ...
    static public function getLanguagById($id_lang)
    {
    ...
    static public function getIsoLangById($id_lang)
	{
        $lang = Language::getLanguagById($id_lang);
        return $lang["iso_lang"];
	}
    ...
    /**
	  * Return id from iso lang
	  *
	  * @param string $iso_code Iso lang
	  * @return integer Language ID
	  */
	static public function getIdByIsoLang($iso_lang)
	{
		$result = Db::getInstance()->getRow('
		SELECT `id_lang`
		FROM `'._DB_PREFIX_.'lang`
		WHERE `iso_lang` = \''.pSQL($iso_lang).'\'');
		if (isset($result['id_lang']))
			return intval($result['id_lang']);
	}
