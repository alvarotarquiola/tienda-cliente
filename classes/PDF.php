...
class PDF extends PDF_PageGroup
{
	...
	*/
	public function Header()
	{
		...
		elseif (self::$order->getCurrentState()==_PS_OS_PRESU_)
			$this->Cell(77, 10, self::l('PRESUPUESTO #').' '.Configuration::get('PS_INVOICE_PREFIX', intval($cookie->id_lang)).sprintf('%06d', self::$order->invoice_number), 0, 1, 'R');
	...
	*/
	public static function invoice($order, $mode = 'D', $multiple = false, &$pdf = NULL, $slip = false, $delivery = false)
	{
		...
		if ($order->getCurrentState()==_PS_OS_PRESU_){
			$pdf->Cell($width, 10, self::l('Presupuesto'), 0, 'L');
		}else{
		...
		elseif ($order->getCurrentState()==_PS_OS_PRESU_)
			$pdf->Cell(0, 6, self::l('PRESUPUESTO #').Configuration::get('PS_INVOICE_PREFIX', intval($cookie->id_lang)).sprintf('%06d', self::$order->invoice_number).' '.self::l('from') . ' ' .Tools::displayDate(self::$order->invoice_date, self::$order->id_lang), 1, 2, 'L', 1);
		else
	...
	*/
	...
	{
		global $cookie;//$cookie->id_lang
		if (@!include(_PS_TRANSLATIONS_DIR_.Language::getIsoById($cookie->id_lang_factura).'/pdf.php'))
			die('Cannot include PDF translation language file : '._PS_TRANSLATIONS_DIR_.Language::getIsoById($cookie->id_lang_factura).'/pdf.php');
