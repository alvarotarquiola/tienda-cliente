...
class		OrderHistory extends ObjectModel
{
    ...
    public static function upadateQuantityProducto($id_order, $new_order_state)
    {
        $id_lang = Language::getIdByIso("es");
        $orderStateLang = new OrderState($new_order_state, $id_lang);
        
        if($orderStateLang->name == "5-Cancelar")
        {            
            $there_state = OrderHistory::thereOrderState($id_order, $orderStateLang->id);   
            if(!$there_state)
            {
                $orderDetail = OrderDetail::getByIdOrder($id_order);

                $total_quantity = 0;
                $total_quantity_missing = 0;
                $product = new Product($orderDetail["product_id"]);
                $aux_quantity = $product->quantity;
                $aux_quantity_missing = $product->quantity_missing;
                $product_quantity = $orderDetail["product_quantity"];
                $product_quantity_in_stock = $orderDetail["product_quantity_in_stock"];
                $product_quantity_dif = $product_quantity - $product_quantity_in_stock;
                
                if($product_quantity == $product_quantity_in_stock){
                    $total_quantity = ($aux_quantity) + ($product_quantity);
                    Product::updateProductQuantity($orderDetail["product_id"], $total_quantity);
                }elseif($product_quantity > 0 && $product_quantity_in_stock > 0){
                    $aux_res = $product_quantity - $product_quantity_in_stock; 
                    $total_quantity = ($aux_quantity) + ($product_quantity_in_stock);
                    $total_quantity_missing = $aux_quantity_missing - $aux_res;
                    Product::updateProductQuantity($orderDetail["product_id"], $total_quantity);
                    Product::updateProductQuantityMissing($orderDetail["product_id"], $total_quantity_missing);
                }elseif($product_quantity > 0 && $product_quantity_in_stock == 0){
                    $total_quantity_missing = ($aux_quantity_missing) - ($product_quantity);
                    Product::updateProductQuantityMissing($orderDetail["product_id"], $total_quantity_missing);
                } 
            }else{
            }
    ...
    static public function thereOrderState($id_order, $id_order_state)
    {
        $res = false;
		...
		WHERE `id_order` = '.intval($id_order).' AND `id_order_state` = '.intval($id_order_state));

        if($result["id_order_history"] != null || $result["id_order_history"] != "")
        {
            $res = true;
        }
        return $res;
