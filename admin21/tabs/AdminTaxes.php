...
class AdminTaxes extends AdminTab
{
	...
	public function displayForm($isMainTab = true)
	{
        ...
        echo '
            <script type="text/javascript">
				$(document).ready(function() {
                    jQuery("#form_taxes").find(".text_taxes_form input").blur(function(){
                       var textlang = jQuery(this).attr("value");
                       $(".text_taxes_form2").each(function(index){
		...
		echo '
		<form action="'.$currentIndex.'&submitAdd'.$this->table.'=1&token='.$this->token.'" method="post" id="form_taxes">
		'.($obj->id ? '<input type="hidden" name="id_'.$this->table.'" value="'.$obj->id.'" />' : '').'
			<fieldset class="width3"><legend><img src="../img/admin/dollar.gif" />'.$this->l('Taxes').'</legend>
				...
				foreach ($this->_languages as $language):
                    echo '
					<div id="name_'.$language['id_lang'].'" class="'.($language['id_lang'] == $this->_defaultFormLanguage ? 'text_taxes_form' : 'text_taxes_form2').'" style="display: '.($language['id_lang'] == $this->_defaultFormLanguage ? 'block' : 'none').'; float: left;">
						<input size="33" type="text" class="text_taxes_form" name="name_'.$language['id_lang'].'" value="'.htmlentities($this->getFieldValue($obj, 'name', intval($language['id_lang'])), ENT_COMPAT, 'UTF-8').'"/><sup> *</sup>
				...
				if (isset($id) AND !empty($id))
				{
					if ($this->tabAccess['edit'] === '1')
					{
						...
						{   
                            ...
                            //se aumento para resetaer los campos tax de idioma.
                            /*$object->name[1] = $object->name[3];
                            $object->name[2] = $object->name[3];
                            $object->name[4] = $object->name[3];
                            $object->name[5] = $object->name[3];*/
