...
class AdminPayment extends AdminTab
{
	...
	public function postProcess()
	{
		...
		elseif (Tools::isSubmit('submitPagoConfirmacion'))
			$this->savePago();	
	}
	...
	public function displayPagoConfirmacion()
	{
		global $currentIndex;
		...
		echo '<form action="'.$currentIndex.'&token='.$this->token.'" method="post" class="width3" id="form_pagoconfirmacion">';
		echo '<fieldset><legend>'.$this->l('Pay x Confirmation').'</legend>';
		
		echo '<table cellpadding="0" cellspacing="0" class="table width3">';
		
		echo '<tr><td>'.$this->l('On out of stock').'</td>';
		echo '<td><input type="radio" name="pxc" value="Afs" '.$checked1.'></td></tr>';
	
		echo '<tr><td>'.$this->l('Always On').'</td>';
		echo '<td><input type="radio" name="pxc" value="As" '.$checked2.'></td></tr>';
	
		echo '<tr><td>'.$this->l('Off').'</td>';
		echo '<td><input type="radio" name="pxc" value="D" '.$checked3.'></td></tr>';
	
		echo '</table>';
		echo '<div style="text-align:center;"><input type="submit" class="button space" name="submitPagoConfirmacion" value="'.$this->l('Save').'" /></div>';
		echo '</fieldset>';
		echo '</form>';
	}// fin function displayPagoConfirmacion 
	
	public function savePago()
	{
		
		if($_POST["pxc"]){
			Configuration::updateValue('PS_pxc',$_POST["pxc"]);
