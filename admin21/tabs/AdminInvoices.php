...
class AdminInvoices extends AdminTab
{
	...
	public function displayForm($isMainTab = true)
	{
        ...
        if(!isset($cookie->id_lang_factura)){
            $cookie->id_lang_factura = $cookie->id_lang;
            //echo "ID LANG ".$cookie->id_lang_factura;
        }

        echo '<h2>'.$this->l('Print PDF invoices').'</h2>';
    
        echo '<fieldset class="width2">
                <form action="'.$currentIndex.'&submitPrint=1&token='.$this->token.'" method="post">
    				...
    				<div class="margin-form">
    					<input type="submit" value="'.$this->l('Generate PDF file').'" name="submitPrint" class="button action_submit_form" />
    				</div>
