...
{if $category->id AND $category->active}
	...
	{foreach from=$subcategories item=subcategory}
		{if !$subcategory.id_image OR strpos($link->getCatImageLink($subcategory.link_rewrite, $subcategory.id_image, 'medium'),"default-medium.jpg") == 1}
			{$no_image = 1}
		{/if}
	{/foreach}
	{if !no_image}
			...
			{foreach from=$subcategories item=subcategory}
				<li>
					<a href="{$link->getCategoryLink($subcategory.id_category, $subcategory.link_rewrite)|escape:'htmlall':'UTF-8'}" title="{$subcategory.name|escape:'htmlall':'UTF-8'}">
						{if $subcategory.id_image AND strpos($link->getCatImageLink($subcategory.link_rewrite, $subcategory.id_image, 'medium'),"default-medium.jpg") == 0}
