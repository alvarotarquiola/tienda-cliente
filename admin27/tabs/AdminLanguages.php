...
class AdminLanguages extends AdminTab
{
	public function __construct()
	{
        ...
        'iso_lang' => array('title' => $this->l('ISO lang'), 'width' => 80, 'align' => 'center'),
	...
	public function displayListContent($token=NULL)
	{
		...
		if ($this->_list)
			...
			foreach ($this->_list AS $tr)
			{
                ...
                $lang = Language::getLanguage($tr["id_lang"]);
	...
	public function displayForm($isMainTab = true)
	{
        ...
        $array_lang = array("Lang-1", "Lang-2", "Lang-3", "Lang-4", "Lang-5");
        
        echo '
            <script type="text/javascript">
                jQuery(function(){
                    jQuery(".margin-form #iso_lang").change(function(){
                        var ele = jQuery(this);
                        var aux = ele.attr("iso_lang");
                        if(ele.val() != aux)
                        {
                            jQuery.post("ajax.php", {"new_iso_lang": ele.val(), "iso_lang": aux}, function(res){
                                var obj = jQuery.parseJSON(res);
                                if(obj.message == "error"){
                                    //alert("El ISO lang "+obj.iso_lang+" ya existe.");
                                    var msg = "El ISO lang "+obj.iso_lang+" ya existe."
                                    jQuery("#error_iso_lang").text(msg);   
                                }
                            });    
                        }
                    });
		...
		'.($obj->id ? '<input type="hidden" name="id_'.$this->table.'" value="'.$obj->id.'" />' : '').'
			<fieldset><legend><img src="../img/admin/world.gif" />'.$this->l('Languages').'</legend>
                ...
                <label>'.$this->l('ISO lang:').' </label>
				<div class="margin-form">
                    <select name="iso_lang" id="iso_lang" iso_lang="'.$this->getFieldValue($obj, 'iso_lang').'">
                        <option value="">  '.$this->l("Seleccione").'  </option>';
                    foreach($array_lang as $lang):
                        echo '<option value="'.strtolower($lang).'"';
                        echo $this->getFieldValue($obj, 'iso_lang') == strtolower($lang) ? ' selected="selected"' : '' ;
                        echo '>'.$lang.'</option>';
                    endforeach;
         echo  '            
                    </select><sup>*</sup>
                    <p style="color: red;" id="error_iso_lang"></p>
				</div>
